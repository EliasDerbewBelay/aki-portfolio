  ...
  return (
    <main className="min-h-screen p-8">
      <header className="max-w-6xl mx-auto mb-12 text-center">
        <h1 className="text-6xl font-black text-gray-900 mb-4 tracking-tighter">SELECTED WORKS</h1>
          ...
          <Link href={`/projects/${project.slug}`} key={project.id} className="group">
            ...
            <div className="mt-4 flex justify-between items-start">
              <div>
                ...
                <p className="text-gray-500 text-sm">{project.category_display}</p>
              </div>
              {project.video_url && <PlayCircle className="w-5 h-5 text-gray-400" />}
            </div>
          </Link>
        ))}
      </div>
